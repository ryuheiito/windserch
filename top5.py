...
for index, row in df_excel.iterrows():
    ...
    if os.path.isfile(csv_path):
        ...
        # 最大風速ランキングが1から5のデータを抽出
        df_filtered = df_csv[df_csv["最大風速ランキング"].between(1, 5)]
        
        # 最大風速ランキングの数値が重複した場合は年が大きいものを優先
        df_filtered = df_filtered.sort_values(by=["最大風速ランキング", "年"], ascending=[True, False])
        
        # 最大風速ランキングが1から5の最新のデータを取得
        if not df_filtered.empty:
            latest_data = df_filtered.groupby("最大風速ランキング").first().reset_index()
            
            # データをリストに追加
            for _, row in latest_data.iterrows():
                data_list.append([no, station_name, row["年"], row["最大風速ランキング"], row["最大風速"]])
    else:
